...
float field_width = 6;
#define FLOAT_FORMAT std::fixed << std::setprecision(6) << std::setw(field_width)
...
typedef void fuse_function(quaternion &rotation, double dt, const vector &angular_velocity,
                           const vector &acceleration, const vector &magnetic_field);

void fuse_compass_only(quaternion &rotation, double dt, const vector &angular_velocity,
                       const vector &acceleration, const vector &magnetic_field) {
...
void ahrs(IMU &imu, fuse_function *fuse, rotation_output_function *output) {
	...
	while (1) {
		...
		vector acceleration = imu.readAccel();
